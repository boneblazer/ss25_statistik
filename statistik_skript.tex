...
\begin{equation*}
    ...
    \end{cases}
\end{equation*}
$S_n$ = Anzahl der K-Würfe $\Rightarrow$ Mögliche Werte von $S_n:0,1,\dots,n.$ 
...
\begin{itemize}
\item[] $\sqrt{\frac{1}{n}\sum_{i=1}^{n}(X_i-\mu)^2}$ falls $\mu$ bekannt
...
\noindent Allgemeiner Ansatz: $\alpha\in(0,1)$
\[A=a(X_1,\dots,X_n), B=b(X_1,\dots,X_n)\]
$[A,B]$ ist ein $(1-\alpha)$-Konfidenzintervall für $\vartheta$, falls
\[P(A\leq\vartheta\leq B)=1-\alpha\]
symmetrisch, falls
\[P(A>\vartheta)=P(B<\vartheta)\]

\noindent Die Realisierung des Konfidenzintervalls für Beobachtungswerte $x_1,\dots,x_n$ ist das Schätzintervall
\[a(x_1,\dots,x_n,), b(x_1,\dots,x_n)\]
Konstruiere Konfidenzintervall für den unbekannten Parameter $\mu$ einer N($\mu,\sigma^2$)-Verteilung
\[\text{\underline{Fall A: $\sigma^2$ bekannt}}\]
Benutze folgende Eigenschaften: 
\begin{itemize}
\item[(i)] Sind $X_1,\dots,X_n$ unabhängig und jeweils $\sim$N($\mu,\sigma^2$), dann
\[\bar{X_n}=\frac{x_1+\dots+x_n}{n}\sim\text{N}(\mu,\frac{\sigma^2}{n}) \text{ vgl. Tut. 9, A49 für }n=2\]
und 
\[\sqrt{n}\frac{\bar{X_n}-\mu}{\sigma}\sim\text{N}(0,1)\]
\item[(ii)] Sei $\Phi$ die VF eines N($0,1$)-verteilten ZV
\newline Dann gibt es für jedes $0<\alpha<1$ eine eindeutige Zahl $x_\alpha\in\mathbb{R}$, so dass $\Phi(x_\alpha)=\alpha$. 
$x_\alpha$ heißt das $\alpha$\textit{-Quantil} oder $\alpha$\textit{-Fraktil} der N(0,1)-Verteilung. 
\newline Sei nun $X\sim$N($\mu,\sigma_0^2$) ($\mu$ unbekannt, $\sigma_0^2>0$ bekannt). Sei $(X_1,\dots,X_n$) Stichprobe
\newline Konstruiere Konfidenzintervall für $\mu$ symmetrisch zur ML-Schätzfunktion $\bar{X_n}$
\newline Betrachte das Intervall der Form $[\bar{X_n}-\Delta,\bar{X_n}+\Delta]$ und bestimme $\Delta$
\newline Sei $\alpha\in(0,1)$ vorgegebene
\begin{align*}
1-\alpha &= P(\bar{X_n}-\Delta\leq\mu\leq\bar{X_n}+\Delta)\\
&= P(-\Delta\leq\bar{X_n}-\mu\leq\Delta)\\
&= P(\frac{-\sqrt{n}}{\sigma_0}\Delta\leq\sqrt{n}\frac{\bar{X_n}-\mu}{\sigma_0}\leq\frac{\sqrt{n}}{\sigma_0}\Delta)\\
&= \Phi(\frac{\sqrt{n}}{\sigma_0}\Delta)-\Phi(\frac{-\sqrt{n}}{\sigma_0}\Delta)\\
&= 2\Phi(\frac{\sqrt{n}}{\sigma_0}\Delta)-1\\
\Leftrightarrow 1-\frac{\alpha}{2} &= \Phi(\frac{\sqrt{n}}{\sigma_0}\Delta)\\
\end{align*}
Sei c das $(1-\frac{\alpha}{2})$-Quantil der N(0,1)-Verteilung. Dann muss \[c=\frac{\sqrt{n}}{\sigma_0}\Delta \text{ gelten } \Rightarrow \Delta=\frac{c\sigma_0}{\sqrt{n}}\]
\[\Rightarrow [\bar{X_n}-\frac{c\sigma_0}{\sqrt{n}}, \bar{X_n}+\frac{c\sigma_0}{\sqrt{n}}]\]
ist ein Konfidenzintervall für $\mu$ zum Konfidenzniveau $1-\alpha$. 
\newline Es ist symmetrisch, denn
\[P(\bar{X_n}-\frac{c\sigma_0}{\sqrt{n}}>\mu)=P(\sqrt{n}\frac{\bar{X_n}-\mu}{\sigma_0}>c)=1-\Phi(c)=\frac{\alpha}{2}\]
und
\[P(\bar{X_n}+\frac{c\sigma_0}{\sqrt{n}}<\mu)=P(\sqrt{n}\frac{\bar{X_n}-\mu}{\sigma_0}<-c)=\Phi(-c)=1-\Phi(c)=\frac{\alpha}{2}\]
\end{itemize}

\vspace{6pt}
\noindent\textbf{Bsp. 7.1} Sei X$\sim$N($\mu,\sigma^2$) mit $\sigma_0=0,1$. Stichpunkte vom Umfang $n=4$
Ein Schätzintervall für $\mu$ wird wie folgt berechnet: 
\[x_1=5.09,x_2=5.22,x_3=5.02,x_4=5.15\]
\begin{itemize}
\item[1.] Lege Konfidenzniveau $1-\alpha$ fest. 
\[1-\alpha=0,95\]
\item[2.] Bestimme ($1-\frac{\alpha}{2}$)-Quantil c für $\Phi(x)$
\begin{align*}
\alpha=0,05, 1-\frac{\alpha}{2}=0,975\\
\Rightarrow c=\Phi^{-1}(0,975)=1,96\\
\end{align*}
\item[3.] Berechne $\bar{X_n}=\frac{1}{n}\sum_{i=1}^{n}x_i$
\[\bar{X_n}=5,12\]
\item[4.] Berechne $\frac{c-\sigma_0}{\sqrt{n}}$.
\[\frac{1,96-0,1}{\sqrt{4}}=0,098\]
\item[5.] Gebe Schätzintervall
\[[\bar{X_n}-\frac{c-\sigma_0}{\sqrt{n}},\bar{X_n}+\frac{c-\sigma_0}{\sqrt{n}}]\]   
an.
\[[5,022,5,218]\]
\end{itemize}

\[\text{\underline{Fall B: $\mu$ \text{ und } $\sigma^2$ unbekannt}}\]
Sei X$\sim$N($\mu,\sigma^2$) $(X_1,\dots,X_n)$ Stichprobe
\newline Konstruiere Konfidenzintervall für $\mu$ bei unbekanntem $\sigma$: 
Gehe ähnlich vor wie in Fall A, aber ersetze unbekanntes $\sigma$ durch Stichproben-Standardabweichung 
\[S_n=\sqrt{\frac{1}{n-1}\sum_{i=1}^{n}(X_i-\bar{X_n})^2}\]
Benutze, dass die Verteilung der ZV
\[Y=\sqrt{n}=\frac{\bar{X_n}-\mu}{S_n}\]
eine sogenannte (Student'sche) \textit{t-Verteilung mit n-1 Freiheitsgraden} ist, Y$\sim$t($n-1$).
\newline Symm. $(1-\alpha)$-Konfidenzintervall für $\mu$ ist dann geg. durch
\[\bar{X_n}-\frac{aS_n}{\sqrt{n}},\bar{X_n}+\frac{aS_n}{\sqrt{n}}\]

\vspace{6pt}
\noindent\textbf{Bsp. 7.2:} Verwende Stichprobe aus Bsp. 7.1 und bestimme Schätzintervall für unbekannten Parameter $\mu$ einer N($\mu,\sigma^2$)-Verteilung, wenn auch $\sigma$ unbekannt ist.
\begin{itemize}
\item[1.] Lege Konfidenzniveau $1-\alpha$ fest
\[\text{z.B. }1-\alpha=0,95\]
\item[2.] Bestimme $(1-\frac{\alpha}{2})$-Quartil $a$ der t($n-1$)-Verteilung
\[1-\frac{\alpha}{2}=0.975, n-1=3: \text{ Tabelle liefert }a=3.1824\]
\item[3.] Berechne $\bar{x_n}$ und $S_n=\sqrt{\frac{1}{n-1}\sum_{i=1}^{n}(X_i-\bar{X_n})^2}$
\begin{align*}
    \bar{X_4}&=5.12\\
    S_4=\sqrt{\frac{1}{3}\sum_{i=1}^{4}(X_i-5.12)^2}
\end{align*}
\item[4.] Berechne $\frac{a\cdot s_n}{\sqrt{n}}$
\[\frac{3.1824\cdot0.08524}{\sqrt{4}}=0.1356\]
\item[5.] Gebe Schätzintervall
\[[\bar{X_n}-\frac{a\cdot s_n}{\sqrt{n}},\bar{X_n}+\frac{a\cdot s_n}{\sqrt{n}}]\]   
an. 
\[[4.9844,5.2556]\]
\end{itemize}

\section{Testverfahren}
\subsection{Einleitung, Definitionen}
\noindent Formuliere Hypothese $H_0$ über unbekannte Verteilung. Verwende Ergebnis einer Stichprobe,
um zu entscheiden, ob $H_0$ für richtig gehalten wird bzw. abgelehnt wird.
\vspace{6pt}
\noindent\textbf{Bsp. 8.1}
\begin{itemize}
\item[(a)] Gegeben sei eine Münze, $p=P$(\glqq Zahl\grqq)
\begin{center}
Hypothese $H_0: p=0,5$, Münze ist fair
\newline Alternativ $H_1: p\neq0,5$, Münze ist nicht fair
\end{center}
\item[(b)] Sei X$\sim$(N($\mu,\sigma^2$)) mit $\mu$ unbekannt
\begin{center}
Hypothese $H_0:\mu\leq5$
\newline Alternative $H_1:\mu>5$
\end{center}
\end{itemize}

\noindent Mögliche Entscheidungen
\begin{itemize}
\item[]1. $H_0$ wird abgelehnt (und $H_1$ wird akzeptiert)
\item[]2. $H_0$ wird nicht abgelehnt
\end{itemize}
Ein \textit{Test} ist ein Verfahren, das für jedes mögliche Ergebnis der Stichprobe entscheidet, ob $H_0$ abgelehnt wird oder nicht. 
\newline Mögliche Fehler:
\begin{itemize}
    \item[]Fehler 1. Art: $H_0$ wird abgelehnt, obwohl $H_0$ richtig ist.
    \item[]Fehler 2. Art: $H_1$ wird nicht abgelehnt, obwohl $H_0$ falsch ist.
\end{itemize}
Die W'keiten für beide Fehlerarten lassen sich im Allgemeinen nicht gleichzeitig kontrollieren, wenn der Stichprobenumfang gegeben ist. 
$\Rightarrow$ Man gibt also eine obere Schranke $\alpha \hspace{6pt} (0<\alpha<1)$ für die W'keit eines Fehlers 1. Art vor und minimiert die andere W'keit unter dieser Bedingung.
\[\alpha \text{ heißt \textit{(Signifikanz-)Niveau}}\]
\noindent Typischerweise $\alpha=0,05$ oder $\alpha=0,01$.
\[\text{\underline{Allgemeine Testsituation}}\]
$\vartheta\in\Theta$ Parameter der Verteilung einer ZV X
\newline Nullhypothese $H_0$: $\vartheta\in\Theta_0$, wobei $\Theta_0\subset\Theta, \hspace{15pt}\emptyset\neq\Theta_0\neq\Theta$.
\newline Alternative $H_1$: $\vartheta\in\Theta_1:=\Theta\backslash\Theta_0$

\vspace{6pt}
\noindent In Bsp. 8.1(a): $\Theta=[0,1]$
\begin{align*}
H_0: p=\frac{1}{2} \hspace{20pt}&\Theta_0=\{\frac{1}{2}\}\\
H_1: p\neq\frac{1}{2} \hspace{20pt}&\Theta_1=\Theta\backslash\Theta_0=[0,\frac{1}{2})\cup(\frac{1}{2},1]\\
\end{align*}
$(X_1,\dots,X_n)$ sei Zufallsstichprobe. $X_1,\dots,X_n$ unabhängige ZV, die jeweils dieselbe Verteilung haben wie X. 
\newline Sei B = Menge aller möglichen Realisationen von $(X_1,\dots,X_n)$, also $B\subset\mathbb{R}^n$

\begin{itemize}
    \item[]\textbf{Test} $\tau$ wird eindeutig beschrieben durch die Menge $K=K_{\tau}\subset\text{B}$
    Lehne $H_0$ genau dann ab, wenn $(X_1(\omega),\dots,X_n(\omega))\in K$
    \newline K heißt \textit{kritischer Bereich} oder \textit{Ablehnungsbereich}
    \item[] t heißt \textbf{\textit{Test zum Niveau}} $\alpha \hspace{6pt}(0<\alpha<1)$, falls
    \[P_{\vartheta}(\tau \text{ lehnt } H_0 \text{ ab})=P_{\vartheta}((X_1,\dots,X_n)\in K)\leq\alpha \text{ f"ur alle }\vartheta\in\Theta_0\]
    Beachte: $\vartheta\in\Theta_0\Rightarrow \hspace{6pt} H_0$ trifft zu
        \newline $\Rightarrow$ W'keit eines Fehlers 1. Art 
        \[=P_{\tau}(\vartheta)=P_{\vartheta}(\tau \text{ lehnt } H_0 \text{ ab}) \leq\alpha\]
    \item[] Die Funktion
        \[\beta_{\tau}(\vartheta):=P_{\vartheta}(\tau \text{ lehnt } H_0 \text{ ab}), \vartheta\in\Theta\]
        heißt \textbf{\textit{Gütefunktion}} von $\tau$. 
        \newline $\tau$ ist Test zum Niveau $\alpha \Leftrightarrow \beta_{\tau}(\vartheta)\leq\alpha$ für alle $\vartheta\in\Theta_0$. 
        \newline Erwünscht: Unter dieser Bedingung sollte $\beta_2(\vartheta)$ für $\vartheta\in\Theta_1$ möglichst groß sein, da dann W'keit für Fehler 2. Art möglichst klein wird. Denn: 
        $\vartheta\in\Theta_1\Rightarrow H_0$ falsch 
        \newline $\Rightarrow$ W'keit für Fehler 2. Art
        \[P_{\vartheta}(\tau \text{ lehnt } H_0 \text{ nicht ab})=1-\beta_{\tau}(\vartheta)\]
\end{itemize}

\subsubsection{Satz 8.1} 
Seien $\tau_1,\tau_2$ Tests zum Niveau $\alpha$ für $H_0:\vartheta\in\Theta_0$ gegen $H_1:\vartheta\in\Theta_1$ mit kritischen Bereichen $K_1,K_2$. 
\newline Gilt $K_1\subset K_2$, dann folgt $\beta_{\tau_1}(\vartheta)\leq\beta_{\tau_2}(\vartheta)$ für alle $\vartheta\in\Theta$

\vspace{2pt}
\noindent\textbf{Beweis:}
\begin{align*}
\beta_{\tau_1}(\vartheta) &= P_{\vartheta}(\tau_1 \text{ lehnt } H_0 \text{ ab}) = P_\vartheta((X_1,\dots,X_n)\in K_1)\\
K_1\subset K_2 &\leq P_\vartheta((X_1,\dots,X_n)\in K_2) = P_{\vartheta}(\tau_2 \text{ lehnt } H_0 \text{ ab}) = \beta_{\tau_2}(\vartheta)\\
\end{align*}
\textbf{Fazit:} Wähle bei geg. Niveau den kritischen Bereich möglichst groß.

\subsection{Testen von Hypothesen über den Mittelwert einer N($\mu,\sigma^2$)-Verteilung}
\noindent\textbf{Fall A: $\mu$ unbekannt, $\sigma^2$ bekannt}
\newline $X\sim$N($\mu,\sigma^2$), $\hspace{25pt} X_1,\dots,X_n$ unabhängig, jeweils $\sim$N($\mu,\sigma^2$)
\begin{align*}
H_0:\mu&\leq\mu_0\\
H_1:\mu&>\mu_0\\
\end{align*}
wobei $\mu_0\in\mathbb{R}$ gegeben ist. Sei $\alpha\in(0,1)$ vorgegebenes Niveau

\vspace{6pt}
\noindent Idee: Lehne $H_0$ ab, falls $\bar{X_n}$ \glqq zu groß\grqq.

\vspace{6pt}
\noindent Betrachte kritischen Bereich der Form
\[K=\{(x_1,\dots,x_n)\in\mathbb{R}^n:\frac{1}{n}(x_1+\dots+x_n)>\gamma\}\]
Bestimme $\gamma$: 
\newline Niveaubedingung: (\textit{die W'keit, dass fälschlicherweise abgelehnt wird})
\[(*) P_{\mu}(\bar{X_n}>\gamma)\leq\alpha \text{ für alle }\mu\leq\mu_0\]
Wähle K so groß wie möglich, also $\gamma$ so klien wie möglich unter der Bedingung (*) (Satz 8.1)

\vspace{6pt}
\noindent Vereinfache die Bedingung
\newline Für jedes $\gamma\in\mathbb{R}$ ist 
\[P_\mu(\bar{X_n}>\gamma)=P_\mu(\frac{\bar{X_n}-\mu}{\sigma/\sqrt{n}}>\frac{\gamma-\mu}{\sigma/\sqrt{n}})=1-\Phi(\frac{\gamma-\mu}{\sigma/\sqrt{n}})\]
streng monoton wachsend in $\mu$. 

\vspace{6pt}
\noindent Daher gilt
\[(*) \Leftrightarrow (**) P_{\mu_0}(\bar{X_n}>\alpha)\leq \alpha\]
Das kleinste $\gamma\in\mathbb{R}$, das (**) erfüllt, ist gegeben durch 
\[P_{\mu_0}(\bar{X_n}>\gamma)=\alpha \hspace{15pt} (\text{da }P_{\mu_0}(\bar{X_n}>\gamma)\text{ monoton fallend in }\gamma)\]
Auflösen nach $\gamma$:
\begin{align*}
\alpha &= P_{\mu_0}(\bar{X_n}>\gamma)=1-\Phi(\frac{\gamma-\mu_0}{\sigma/\sqrt{n}})\\
&\Leftrightarrow \Phi(\sqrt{n}\frac{\gamma-\mu_0}{\sigma})=1-\alpha\\
&\Leftrightarrow \sqrt{n}\frac{\gamma-\mu_0}{\sigma} = \Phi^{-1}(1-\alpha)=:u_{1-\alpha}\\
&\Leftrightarrow \gamma=\mu_0+\frac{\sigma}{\sqrt{n}}u_{1-\alpha}
\end{align*}

\vspace{6pt}
\noindent Test zum Niveau $\alpha$ für das Testen von
\[H_0:\mu\leq\mu_0 \text{ gegen } H_1:\mu>\mu_0\]
\textbf{lehnt $H_0$ genau dann ab, wenn}
\[\sqrt{n}\frac{\bar{X_n}-\mu_0}{\sigma}>u_{1-\alpha},\]
wobei $u_{1-\alpha}$ das ($1-\alpha$)-Quantil der N(0,1)-Verteilung ist.

\vspace{6pt}
\noindent Ähnlich zeigt man für vorgegebenes $\mu_0\in\mathbb{R},0<\alpha<1$ und bekanntes $\sigma>0$
...
\textbf{lehnt $H_0$ genau dann ab, wenn}
\[\sqrt{n}\frac{\bar{X_n}-\mu_0}{\sigma}<u_{1-\alpha},\]
wobei $\Phi(u_{1-\alpha})=1-\alpha$.

\vspace{6pt}
\noindent Test zum Niveau $\alpha$ für das Testen von
\[H_0:\mu=\mu_0 \text{ gegen } H_1:\mu\neq\mu_0\]
\textbf{lehnt $H_0$ genau dann ab, wenn}
\[\sqrt{n}\frac{|\bar{X_n}-\mu_0|}{\sigma}>u_{1-\frac{\alpha}{2}},\]
wobei $\Phi(u_{1-\frac{\alpha}{2}})=1-\frac{\alpha}{2}$.

\noindent Die drei Tests heißen \textit{Einstichproben-Gauß-Tests}.

\vspace{6pt}
\noindent\textbf{Bsp. 8.2:} Sei $X\sim$N($\mu,0.16$)
\newline Eine Stichprobe vom Umfang n=10 liefert für das Stichprobenmittel den Wert
\[\bar{x_{10}=50,294}\]
Teste
\[H_0:\mu=50\text{ gegen }H_1:\mu\neq50\]
zum Niveau $\alpha=0.05$. 
\newline Hier ist $\mu_0=50, \sigma=\sqrt{0,16}=0,4, u_{1-\frac{\alpha}{2}}=u_{0,975}=1,96$.
\begin{align*}
\Rightarrow \sqrt{n}\frac{|\bar{X_{10}}-\mu_0|}{\sigma} &= \frac{0.294}{0.4}\sqrt{10}\\
&= 2.324>1.96=u_{1-\frac{\alpha}{2}}\\
\Rightarrow& \text{ lehne }H_0\text{ ab.}
\end{align*}

\vspace{6pt}
\noindent\textbf{Fall B: $\mu$ und $\sigma^2$ unbekannt}
Ersetze im Gauß-Test
\[\sigma \text{ durch } S_n=\sqrt{\frac{1}{n-1}\sum_{i=1}^{n}(X_i-\bar{X_n})^2}\]
und Quantil der N(0,1)-Verteilung durch Quantil der t($n-1$)-Verteilung. 
\newline Dies liefert im Fall $\sigma$ unbekannt.

\vspace{6pt}
Test zum Niveau $\alpha$ für das Testen von 
\begin{itemize}
\item[(a)] $H_0:\mu\leq\mu_0$ gegen $H_1:\mu>\mu_0$
\item[(b)] $H_0:\mu\geq\mu_0$ gegen $H_1:\mu<\mu_0$
\item[(c)] $H_0:\mu=\mu_0$ gegen $H_1:\mu\neq\mu_0$ 
\end{itemize}
lehnt $H_0$ genau dann ab, wenn
\begin{itemize}
\item[(a)]$\sqrt{n}\frac{\bar{X_n}-\mu_0}{S_n}>v_{1-\alpha}$
\item[(b)]$\sqrt{n}\frac{\bar{X_n}-\mu_0}{S_n}<-v_{1-\alpha}$
\item[(c)]$\sqrt{n}|\frac{\bar{X_n}-\mu_0}{S_n}|>v_{1-\frac{\alpha}{2}}$ 
\end{itemize}
wobei $v_{1-\alpha}$ (bzw. $v_{1-\frac{\alpha}{2}}$) das $1-\alpha$ (bzw. $1-\frac{\alpha}{2}$)-Quantil der t(n-1) Verteilung ist.
\newline Diese Tests heißen (Einstichproben-)\textit{t-Tests}.

\vspace{6pt}
\noindent\textbf{Bem. 8.1:} Signifikanzniveau $\alpha$ kontrolliert (ist eine obere Schranke für) \underline{nur} die 
\[\text{W'keit $H_0$ abzulehnen, wenn $H_0$ richtig ist (Fehler 1. Art).}\]
Die W'keit 
\[\text{$H_0$ nicht abzulehnen, wenn $H_0$ falsch ist (Fehler 2. Art)}\]
ist nicht kontrolliert.
\newline Wähle daher $H_0$ so, dass Fehler 1. Art der Gravierendere ist. 

\vspace{6pt}
\noindent Möchte man eine Aussage \textbf{\glqq statistisch widerlegen\grqq}, wählt man die Aussage als \textbf{$H_0$}.

\noindent Möchte man eine Aussage \textbf{\glqq statistisch nachweisen/untermauern/bestätigen\grqq}, wählt man die Aussage als \textbf{$H_1$}.

\vspace{6pt}
\noindent Wird $H_0$ abgelehnt, ist $H_1$ statistisch nachgewiesen zum Niveau $\alpha$. 
Man sagt das Testergebnis ist statistisch signifikant zum Niveau $\alpha$. 

\noindent Wird $H_0$ nicht abgelehnt, heißt das Testergebnis statistisch nicht signifikant zum Niveau $\alpha$. Es gilt nicht \glqq $H_0$ ist verifiziert\grqq. 

\vspace{6pt}
\noindent\textbf{Bem. 8.2:} 
Falls für einen gegebenen Datensatz $\alpha_0\in(0,1)$ so ist, dass 
\[H_0 \text{ zum Niveau } \alpha \text{ abgelehnt wird }\forall\alpha>\alpha_0\]
und 
\[H_0 \text{ zum Niveau }\alpha \text{ nicht abgelehnt wird }\forall\alpha<\alpha_0\]
dann heißt $\alpha_0$ \textit{p-Wert} oder empirisches Signifikanzniveau. p-Wert kann als
Meß für die Glaubwürdigkeit von $H_0$ interpretiert werden. Je kleiner der p-Wert, desto weniger glaubwürdig ist $H_0$.

\vspace{6pt}
\noindent In Bsp. 8.3, $H_0:\mu=50$ wird zum Niveau $\alpha$ genau dann abgelehnt, wenn 
\begin{align*}
    2.324 &> u_{1-\frac{\alpha}{2}}\\
    \Leftrightarrow \Phi(2.324) &> \Phi(u_{1-\frac{\alpha}{2}})=1-\frac{\alpha}{2}\\
    \Leftrightarrow \alpha &> 2(1-\Phi(2.324))=0.02\\
\end{align*}


\subsection{Der $\chi^2$-Anpassungstest}
Sind $Z_1,\dots,Z_n$ unabhängig N($0,1$)-verteilte ZV, dann heißt die Verteilung von 
\[V:=Z_1^2+\dots+Z_n^2\]
\textit{Chi-Quadrat-Verteilung mit n Freiheitsgraden}. ($V\sim\chi^2(n)$)

\vspace{6pt}
\noindent\textbf{Testproblem}
\newline Sei $X$ eine diskrete ZV mit bekannten möglichen Werten\[a_1,\dots,a_k\] und unbekannten W'keiten \[p_i=P(X=a_i), \hspace{10pt} 1\leq i\leq k\]
Teste
\[H_0:p_i=\bar{p_i} \text{ für }i=1,\dots,k\]
wobei $\bar{p_1},\dots,\bar{p_k}>0$ gegebene Zahlen sind mit $\bar{p_1}+\dots+\bar{p_k}=1$.
\newline Sei dazu $(X_1,\dots,X_n)$ eine Zufallsstichprobe und 
\begin{align*}
N_i: &= |\{j\in\{1,\dots,n\}:X_j=a_j\}|\\
&= \text{ absolute Häufigkeit des Werts $a_i$ in der Stichprobe}\\
\end{align*}
für $i=1,\dots,k$.
\newline Ist $H_0$ richtig, dann ist E$(N_i)=n\bar{p_i}$ und $(N_i-n\bar{p_i})^2$ sollte nicht "zu groß" sein.
\newline Theorie liefert folgende Aussage für die Statistik
\[Q:=\sum_{i=1}^{k}\frac{(N_i-n\bar{p_i})^2}{n\bar{p_i}}\]
Ist 
\[n\bar{p_i}\geq5, \hspace{10pt}1\leq i\leq k\]
dann hat Q annähernd eine $\chi^2(k-1)$-Verteilung, falls $H_0$ richtig ist.
\newline Ist also $x_{1-\alpha}$ das ($1-\alpha$)-Quantil der $\chi^2(k-1)$-Verteilung, so ist 
\[P(Q\leq x_{1-\alpha})\approx 1-\alpha\]
und 
\[P(Q>x_{1-\alpha})\approx\alpha,\hspace{10pt} \text{ falls $H_0$ richtig ist.}\]
Das Verfahren 
\[\text{Lehne $H_0$ genau dann ab, wenn }Q>X_{1-\alpha}\]
ist also ein (approximativer) Test zum Testen von $H_0$ zum Niveau $\alpha$. 
\newline Der Test ist ein $\chi^2$-Anpassungstest.

\vspace{6pt}
\noindent\textbf{Bsp. 8.3:} 
\newline Teste Echtheit eines Würfels zum Niveau $\alpha$=0.01. Hier ist
\begin{align*}
a_1=1,\dots,A_6=6,& \hspace{10pt}k=6\\
\bar{p_i}=\frac{1}{6},& \hspace{10pt}1\leq i\leq 6\\
H_0:p_i=\frac{1}{6},& \hspace{10pt}i=1,\dots,6\\
\end{align*}
In $n=120$ Würfen waren die Häufigkeiten der 6 Seiten
\[n_1=29,n_2=11,n_3=26,n_4=12,n_5=27,n_6=15\]
$n\bar{p_i}=20\geq 5$ für alle $i \hspace{10pt} \Rightarrow \chi^2$-Test anwenden. 
\begin{align*}
q &= \sum_{i=1}^{6}\frac{(N_i-n\bar{p_i})^2}{n\bar{p_i}}=\frac{1}{20}\sum_{i=1}^{6}(n_i-20)^2=\frac{1}{20}(9^2+9^2+6^2+8^2+7^2+5^2)\\
&= 16,8
\end{align*}
$(1-\alpha)$-Quantil der $\chi^2(5)$-Verteilung: $X_0,99=15,09$
\[q=16.9>x_0,99\]
Lehne Nullhypothese \glqq Würfel ist echt\grqq ab.
